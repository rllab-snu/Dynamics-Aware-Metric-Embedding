...
# from planet.planet.tools import nested
...
class MiniWrapper(object):
  def __init__(self, env, state_components, _dtype='uint'):
    self._env = env
    self._components = state_components
    self._dtype = _dtype
  def __getattr__(self, name):
    return getattr(self._env, name)
  ...
  def step(self, action):
    ...
    for k in self._components:
      if not observ[k].shape:
        observ[k] = np.array([observ[k]])
      obs_vec.append(observ[k])
    obs_vec = np.concatenate(obs_vec)
    obs_img = observ['image']
    if self._dtype == 'float':
      obs_img = obs_img.astype(np.float32) / 255.
    return (obs_vec, obs_img), reward, done, info
  ...
  def get_obs_dim(self):
    ...
    for k in self._components:
      ...
      if not self._env.observation_space[k].shape:
        dim += 1
      else:
        dim += self._env.observation_space[k].shape[0]
    return dim
